...
const int MAX_THREADS = 3;  // 最大线程数限制

Server& Server::port(int p) {
    m_port = p;
    return *this;
}
...
Server& Server::start() {
    ...
    std::cout << std::format("服务器正在监听端口 {}...", m_port) << std::endl;
    return *this;
}
...
void Server::run() {
    for (;;) {
        ...
        client->socket = accept(m_server_fd, (struct sockaddr*)&client->address, &addr_len);
        ...
        // 检查当前线程数
        {
            ...
            // 如果线程数达到上限，等待
            while (m_threads.size() >= MAX_THREADS) {
                std::cout << std::format("当前活跃线程数: {}", m_threads.size()) << std::endl;
                // 可以在这里添加更复杂的等待逻辑或拒绝新连接
                std::unique_lock unilk(mtx);
                cv.wait_for(unilk, std::chrono::seconds(1));
        ...
        // 创建线程处理客户端
        try {
            m_threads.emplace_back([this, client]() {
            ...
            {
                std::lock_guard<std::mutex> lock(mtx);
                std::cout << "当前活跃线程数: " << m_threads.size() << std::endl;
            }
